...
### NOTE FOR EIKE ####
We modified the mcSimulation() function (the R file) had all the functions together in one file. We split these up into individual R files, including: 
print.summary.mcSimulation.R
print.mcSimuation.R
plsr.mcSimulation.R
summary.mcSimulation.R
hist.mcSimulation.R 
as.data.frame.mcSimulation.R).
# DELETE THIS NOTE ####
...
We added the requested base_size option most recently...
# Delete this note ####
...
# 
# # use the result of multi_EVPI() (i.e. evpi list) to create a full data frame
# full_evpi_data <- NULL
# 
...
#   data <- evpi[[i]]
# 
#   data["output_variable"] <- names(evpi)[i]
# 
#   #merge the data frames
#   if (is.null(full_evpi_data)) full_evpi_data <- data else
# 
...
#     combined_table <- dplyr::left_join(full_evpi_data, input_table, by = c( "variable" = "variable")) else
...
# 
#   # Order the variable or labels according EVPI
# 
#   if ("label" %in% colnames(filtered_table))
# 
#     ordered_vars <- stats::reorder(filtered_table$label, filtered_table$EVPI) else
# 
#       ordered_vars <- stats::reorder(filtered_table$variable, filtered_table$EVPI)
# 
#   # ggplot
# 
# library(ggplot2)
#   # temp with multiple output variables
# ggplot(filtered_table, aes(x = EVPI, y = ordered_vars)) +
#   geom_col() +
#   facet_grid(~output_variable)
# 
#  # with one output variable and facet
# ggplot(dplyr::filter(filtered_table, output_variable == "NPV_decision_do"), aes(x = EVPI, y = variable)) +
#   geom_col() +
#   facet_grid(~variable)
