...
#plot from multi_EVPI just plots the input variables
# shows the y axis of output variables but not of input variables
# multi_EVPI exports a table (if write_table=T) but the output of the function is completely different
#a list of EVPI etc. as columns and outcome variables as rows... 
# The Gonzalo example offers some ideas about how to move ahead but would be easier to have a standard output of multi_EVPI
```
We

```{r}

evpi_data <- as.data.frame(evpi)
evpi_unlist <- unlist(evpi)

outs <- data.frame(variable=rownames(evpi),
                       expected_gain=unlist(evpi[,"expected_gain"]),
                       EVPI_do=unlist(evpi[,"EVPI_do"]),
                       EVPI_dont=unlist(evpi[,"EVPI_dont"]))
