      ...
      if (i == "median") {
        ...
        if (!is.null(median)) {
          ...
          median[!is.na(median) & is.character(median) & median == "mean"] <- rowMeans(
            cbind(lower, upper)
          )[!is.na(median) & is.character(median) & median == "mean"]
...
#'                   posnorm,      50,    2000
#'                   posnorm,      100,   1000"
#' estimateMin <- as.estimate(read.csv(
#'   header = TRUE, text = estimateTextMin,
...
#' estimateMarg <- as.estimate(read.csv(
#'   header = TRUE, text = estimateText,
#'   strip.white = TRUE, stringsAsFactors = FALSE
#' ))
#' print(estimateMarg)
#' print(corMat(estimateMarg))
...
#'                   revenue1,        1,   -0.3
#'                   costs2,       -0.3,      1"
#' estimateCor <- as.estimate(
#'   read.csv(
#'     header = TRUE, text = estimateTextMarg,
#'     strip.white = TRUE, stringsAsFactors = FALSE
#'   ),
#'   correlation_matrix = data.matrix(read.csv(
#'     text = estimateTextCor,
#'     row.names = 1,
#'     strip.white = TRUE
#'   ))
#' )
