...
estimate1d <- function(distribution, lower, upper, ...) {
  ...
  if (!is.null(optionalArguments)) {
    for (i in names(optionalArguments)) {
      ...
      ### Check 1-d property:
      # if ( is.null(optionalArguments[[i]]) )
      #  stop("Optional argument \"", i, "\" is null.") # this generated an error - had to disable it
...
as.estimate1d <- function(x, ...) {
  ...
  estimate1dObject <- estimate1d(
    distribution = x_vec[["distribution"]],
    lower = x_vec[["lower"]],
    upper = x_vec[["upper"]],
    ...
    if (!rho[["distribution"]] == "triang") {
      if (!is.null(median <- rho[["median"]])) {
        ...
        if (!isTRUE(msg <- all.equal(median, median_calc, scale = scale, tolerance = relativeTolerance))) {
          warning(
            "For method=\"calculate\": deviation of calculated \"median\" from supplied target value:\n",
