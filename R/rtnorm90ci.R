...
rtnorm90ci <- function(n, ci, median = mean(ci), lowerTrunc = -Inf, upperTrunc = Inf, method = "numeric", relativeTolerance = 0.05, ...) {
  ...
  if (method == "numeric") {
    param <- paramtnormci_numeric(
      p = p, ci = ci, lowerTrunc = lowerTrunc, upperTrunc = upperTrunc,
      relativeTolerance = relativeTolerance
    )
  } else if (method == "fit") {
    param <- paramtnormci_fit(
      p = p, ci = ci, median = median, lowerTrunc = lowerTrunc, upperTrunc = upperTrunc,
      relativeTolerance = relativeTolerance
    )
  } else {
