...
#' currentEstimate <- as.estimate(read.csv(
...
#' individualEvpiResult <- individualEvpiSimulation(
#'   welfare = profitModel,
...
individualEvpiSimulation <- function(welfare, currentEstimate,
  ...
  for (i in perfectProspectiveNames) {
    ...
    if (!is.null(prospectiveEstimate[[i]]$correlation_matrix)) {
      correlationMatrix <- corMat(prospectiveEstimate[[i]])[
