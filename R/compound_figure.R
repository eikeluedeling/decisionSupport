...
compound_figure <- function(model = NULL,
  ...
  if (!is.null(mcSimulation_object) &
    ...
    monte_carlo <- mcSimulation_object
  } else {
  ...
  distribution_plot <- decisionSupport::plot_distributions(
    mcSimulation_object = monte_carlo,
  ...
  pls_plot <- plot_pls(pls_result,
    input_table = input_table, threshold = 0.8,
  ...
  evpi_plot <- plot_evpi(evpi,
    decision_vars = decision_var_name, input_table = input_table,
    new_names = "",
