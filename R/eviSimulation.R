...
#' revenueConst <- mean(c(
#'   currentEstimate$marginal["revenue", "lower"],
...
#' eviSimulationResult <- eviSimulation(
#'   welfare = profit,
...
#'   list(
#'     Profit = x$revenue - x$costs,
#'     Costs = -x$costs
#'   )
#' }
#' # Calculate the Expected Value of Information:
#' eviSimulationResult <- eviSimulation(
#'   welfare = decisionModel,
#'   currentEstimate = currentEstimate,
#'   prospectiveEstimate = prospectiveEstimate,
#'   numberOfModelRuns = numberOfModelRuns,
#'   functionSyntax = "data.frameNames"
...
#' costsConst <- mean(c(
#'   currentEstimate$marginal["costs", "lower"],
...
#' prospectiveEstimate <- list(
#'   perfectInformationRevenue = perfectInformationRevenue,
...
eviSimulation <- function(welfare, currentEstimate, prospectiveEstimate, numberOfModelRuns,
  ...
  analysisCurrent <- welfareDecisionAnalysis(
  ...
  if (inherits(prospectiveEstimate, "estimate")) {
    ...
    analysisProspective <- welfareDecisionAnalysis(
      estimate = prospectiveEstimate,
      welfare = welfare,
      numberOfModelRuns = numberOfModelRuns,
      randomMethod = randomMethod,
      functionSyntax = functionSyntax,
    ...
    analysisProspective <- lapply(
      X = prospectiveEstimate,
      FUN = function(estimate) {
...
summary.eviSimulation <- function(object,
  ...
  summaryList <- list(
    evi = format(x = object$evi, digits = digits),
