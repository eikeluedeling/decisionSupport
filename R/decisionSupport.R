...
decisionSupport <- function(inputFilePath, outputPath, welfareFunction, numberOfModelRuns,
  ...
  if (write_table) {
    ...
    write.csv(
      data.frame(welfareDecisionResults$mcResult$x, welfareDecisionResults$mcResult$y),
  ...
  if (plsrVipAnalysis) {
    ...
    for (i in names(welfareDecisionResults$mcResult$y)) {
      ...
      color_bars <- chillR::color_bar_maker(vipResult, coef,
        threshold = 0.8,
  ...
  if (!is.null(individualEvpiNames)) {
    ...
    individualEvpiResults <- individualEvpiSimulation(
      welfare = welfareFunction,
    ...
    write.csv(
      sort(summary(individualEvpiResults), along = sortEvpiAlong)$summary$evi,
      individualEvpiSimulationSummaryFilePath
    )
    if (verbosity > 0) {
      cat(
        "IndividualEVPI simulation summary written into file: ",
        individualEvpiSimulationSummaryFilePath, "\n"
      )
    }
  }
