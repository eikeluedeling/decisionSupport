...
multi_EVPI <- function(mc, first_out_var, write_table = FALSE, outfolder = NA) {
  ...
  for (out_var in outvars) {
    ...
    if (write_table) {
      ...
      write.csv(outs, paste(file.path(
        outfolder,
        paste("EVPI_table_",
          out_var, ".csv",
          sep = ""
...
plot.EVPI_outputs <- function(x,
  ...
  if (!length(toplot) == 0) {
    if (scale_results) {
      ...
      if (scaling_factor > 15) {
        toplot <- toplot / 1000000000000000
        suffix <- "(in quadrillions)"
      } else if (scaling_factor > 12) {
        toplot <- toplot / 1000000000000
        suffix <- "(in trillions)"
      } else if (scaling_factor > 9) {
        toplot <- toplot / 1000000000
        suffix <- "(in billions)"
  ...
  if (!length(toplot) == 0) {
    barplot(toplot,
      horiz = TRUE, las = 1, cex.lab = cexexlarge, cex.axis = cexex,
  ...
  if (unique(x[[out_var]][6]) == "do") {
    legend("bottomright",
      legend = paste("Recommendation: do"),
      cex = cexex, bg = "LIGHT GREEN", box.col = "LIGHT GREEN"
    )
  } else {
    legend("bottomright",
      legend = paste("Recommendation: don't"),
