...
#' # Perform the Monte Carlo simulation:
#' predictionProfit2 <- mcSimulation(
...
mcSimulation <- function(estimate, model_function, ..., numberOfModelRuns,
  ...
  x <- random(
    rho = estimate, n = numberOfModelRuns,
    method = randomMethod,
  ...
  if (functionSyntax == "data.frameNames") {
    ...
    if (any(colnames(y) == as.character(seq_len(ncol(y))))) {
      colnames(y) <- paste("output_", seq_len(ncol(y)), sep = "")
    }
