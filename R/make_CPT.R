...
#'
#' make_CPT(
#'   parent_effects = list(c(-1, 1), c(-0.5, 0, 0.5)),
#'   parent_weights = c(3, 1), b = 1.5, child_prior = c(.2, .6, .2), child_states = c("a", "b", "c")
#' )
#'
#' test_CPT <- make_CPT(
#'   parent_effects = list(c(-1, 3), c(-4, 2), c(-2, 3, 4), c(1, 2, 3)),
...
make_CPT <- function(parent_effects, parent_weights, b, child_prior,
  ...
  } else if (!length(ranking_child) == child_n_states) {
    ranking_child <- interpolate_gaps(c(-1, rep(NA, child_n_states - 2), 1))$interp
  }
  ...
  if (length(gamma) == 1) {
    for (i in 1:length(gamma)) CPT <- CPT + gamma[[i]][, tmp]
  } else {
  ...
  leg_tab <- tmp
  if (length(gamma) == 1) {
    leg_tab <- data.frame(matrix(parent_states[[1]], nrow = 1))
  } else {
