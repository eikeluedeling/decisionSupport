...
rdistq_fit <- function(distribution, n, percentiles = c(0.05, 0.5, 0.95), quantiles,
  ...
  if (distribution == "triang") {
    ...
    } else if (!paste("package", requiredPackage, sep = ":") %in% search()) attachNamespace(requiredPackage)
  }
  ...
  capture.output(
    dists <- try(rriskDistributions::rriskFitdist.perc(
      p = percentiles,
      q = quantiles,
      show.output = TRUE,
      tolConv = tolConv,
  ...
  for (j in seq(along = quantiles)) {
    ...
    if (!isTRUE(msg <- all.equal(quantiles[[j]], quantiles_calc[[j]], scale = scale, tolerance = relativeTolerance))) {
      warning(
        "Fitted value of ", 100 * percentiles[[j]], "%-quantile: ", quantiles_calc[[j]], "\n  ",
